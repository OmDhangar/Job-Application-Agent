...
SQLAlchemy 2.0 async ORM — candidates, applications, outreach.
"""
...
class Candidate(Base):
    ...
    # Plain-text extracted from uploaded resume
    resume_raw: Mapped[Optional[str]] = mapped_column(Text)
    # Structured sections: {experience: [...], projects: [...], skills: [...], education: [...]}
    resume_structured: Mapped[Optional[dict]] = mapped_column(JSONB)
    # AI-extracted identity profile (see schemas/candidate.py IdentityProfile)
    ...
    target_companies: Mapped[Optional[list]] = mapped_column(ARRAY(String))
...
class Application(Base):
    ...
    # Lifecycle: discovered → applied → replied → interviewing → offered → rejected | ghosted
    status: Mapped[str] = mapped_column(String, default="discovered", index=True)
    # Stores the tailored resume text (LaTeX or Markdown)
    tailored_resume: Mapped[Optional[str]] = mapped_column(Text)
    # Increments each time resume is re-tailored for this job
    ...
    app_metadata: Mapped[Optional[dict]] = mapped_column("metadata", JSONB)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
...
class Outreach(Base):
    ...
    application: Mapped[Optional["Application"]] = relationship(
        "Application", back_populates="outreach_records"
    )